...
function newOrder(order) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // bruttissima 
        if (restaurantList.find(restaurant => restaurant.id === order.restaurantId) &&
            userList.find(user => user.id === order.userId)) {
            orderPlate.push(...order.orderItems.filter((item) => {
                return restaurantList.find(plate => plate.name === item.name);
            }));
            completeOrder = {
                id: uuid_1.v4(),
                userId: order.userId,
